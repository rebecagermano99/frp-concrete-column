...
class SectionCanvas(QWidget):
    def __init__(self):
        ...
        self.n_inferior = 0
        self.n_superior = 0
        self.diam_inferior = 0.0  
        self.diam_superior = 0.0  
        self.c = 0.0  
    ...
    def paintEvent(self, event):
        ...
        escala = min(avail_w / self.b, avail_h / self.h)
        ...
        offset_px = int(self.c * escala)

        def draw_bar_row(n_barras, diam_mm, y_px):
            if n_barras <= 0:
                return []
            radius_px = max(2, int((diam_mm / 10.0 * escala) / 2))
        ...
        xs_sup = draw_bar_row(self.n_superior, self.diam_superior, y_superior)
        y_inferior = top_left_y + rect_h - offset_px
        xs_inf = draw_bar_row(self.n_inferior, self.diam_inferior, y_inferior)

 
        all_x = xs_sup + xs_inf
        all_y = [y_superior]*len(xs_sup) + [y_inferior]*len(xs_inf)
        if all_x:
            min_x = min(all_x) - offset_px
            max_x = max(all_x) + offset_px
            min_y = min(all_y) - offset_px
            max_y = max(all_y) + offset_px
...
class PilarGUI(QWidget):
    def __init__(self):
        ...
        make_labeled_edit("Força axial (kN):", "Nk", "Insira o valor")
        make_labeled_edit("Excentricidade (cm):", "e", "Insira o valor")
        make_labeled_edit("Largura b (cm):", "b", "Insira o valor")
        make_labeled_edit("Altura h (cm):", "h", "Insira o valor")
        make_labeled_edit("fck (MPa):", "fck", "Insira o valor")
        make_labeled_edit("Módulo E da barra FRP (GPa):", "E", "Insira o valor")
        make_labeled_edit("Cobrimento (cm):", "c", "Insira o valor")
        make_labeled_edit("Resistência compressão da barra FRP (kN/cm²):", "ffd", "Insira o valor")
        make_labeled_edit("Altura do pilar (cm):", "l0", "Insira o valor")
    ...
    def on_calcular(self):
        try:
            ...
            if Mlinha < V:
                lines.append("Seção fora do domínio 5")
            ...
            if milinhad <= misdlim:
                ...
                fiaa = math.sqrt((21 / 4) * (1 - alfa1req))
                ksi1 = (3 / 7) + (8 / (7 * math.sqrt(21 * (1 - alfa1req))))
                ksi1linhaa = (42 * alfa1req - 17.5) / (49 * alfa1req) if alfa1req != 0 else 0.0
                episilonfdlinhaa = 0.0
                sigmafdlinhaa = 0.0
                try:
                    episilonfdlinhaa = (0.002 * (ksi1 - deltalinha1)) / (ksi1 - (3 / 7))
                    sigmafdlinhaa = Ef * episilonfdlinhaa
                except Exception:
                    episilonfdlinhaa = 0.0
                    sigmafdlinhaa = Ef * 0.002

                if sigmafdlinhaa == 0:
                    lines.append("Erro: sigmafdlinhaa = 0 (divisão por zero). Verifique entradas.")
                    self.result_box.setPlainText("\n".join(lines))
                    return
                ...
                lines.append(f"ksi1 = {ksi1:.4f}")
                lines.append(f"eps_fdlinha = {episilonfdlinhaa:.6f}")
                lines.append(f"sigma_fdlinha = {sigmafdlinhaa:.3f}")
                lines.append(f"Areq (área FRP necessária) = {Areq:.3f} cm²")

                phi_cm, n_barras, espac = calc_barras(Areq, b, h, c)
                if phi_cm:
                    lines.append(f"Diâmetro ideal: {phi_cm*10:.1f} mm | {n_barras} barras")
                    lines.append(f"Espaçamento livre: {espac:.2f} cm")
                    self.canvas.n_superior = n_barras
                    self.canvas.diam_superior = phi_cm * 10
                    self.canvas.n_inferior = 0
                    self.canvas.diam_inferior = 0
                else:
                    lines.append("Nenhum diâmetro atende às restrições normativas.")
                    self.canvas.n_superior = 0
                ...
                wb = (ffd / sigmafdblinha) * ((milinhad - 0.85 * (0.5 - deltalinha1)) / (delta1 - deltalinha1)) if (delta1 - deltalinha1) != 0 else 0.0
                Ainf = (wb * b * h * fcd) / ffd if sigmafdblinha != 0 else 0.0

                wlinha = (nid - 0.85 * alfa1b - wb * sigmafdblinha / ffd) * (ffd / sigmafdblinha) if sigmafdblinha != 0 else 0.0
                Asup = (wlinha * b * h * fcd) / ffd if sigmafdblinha != 0 else 0.0


                lines.append(f"Área inferior necessária: {Ainf:.3f} cm²")
                lines.append(f"Área superior necessária: {Asup:.3f} cm²")

                phi_inf, n_inf, espac_inf = calc_barras(Ainf, b, h, c)
                phi_sup, n_sup, espac_sup = calc_barras(Asup, b, h, c)

                if phi_inf and phi_sup:
                    lines.append(f"\n Inferior φ = {phi_inf*10:.1f} mm | {n_inf} barras | espaçamento = {espac_inf:.2f} cm")
                    ...
                    self.canvas.n_inferior = 2
                    self.canvas.diam_inferior = 16
                    self.canvas.n_superior = 2
                    self.canvas.diam_superior = 16
